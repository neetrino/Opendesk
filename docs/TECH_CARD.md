...
| 3.6 | CRON | не нужно | ➖ | |
| 3.7 | Файлы | Cloudflare R2, photo/video до 200 MB | ✅ | presigned PUT, signed GET; in-app camera overlay (gallery on that screen) targets 1080p, hold to start video / tap to stop; swipe-back closes one overlay (media / camera / card); Permissions-Policy camera/microphone self; до 4 файлов в одном комментарии, до 100 файлов в чате карточки; голосовые в чате — inline-плеер с волной, без native `<audio controls>` |
...
- Список доски: первые 10 карточек в каждой колонке без истории чата; дальше по скроллу колонки. Чат карточки: последние 20 сообщений при открытии, старше — по скроллу вверх; поиск по `q` (`GET /api/boards/{boardId}/cards`, `GET /api/boards/{boardId}/cards/{cardId}/comments`)
